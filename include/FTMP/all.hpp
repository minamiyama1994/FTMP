...
#define FTMP_ALL_HPP
namespace ftmp {
template <typename func, typename seq> struct all;
}
...
struct all : foldl<eval<and_<typename lambda<func>::template apply<arg<1> >,
                             id<arg<0> > > >,
                   integral<bool, true>, seq> {};
template <typename func, typename... seq>
struct all<func, set<seq...> > : all<func, list<seq...> > {};
}
#endif