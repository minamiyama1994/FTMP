...
namespace ftmp {
template <typename func, typename a, typename bs> struct foldl;
}
...
struct foldl_helper;
template <typename func, typename a, typename bs1, typename bs2>
struct foldl_helper : foldl<func, typename foldl<func, a, bs1>::type, bs2> {};
}
template <typename func, typename a> struct foldl<func, a, list<> > {
  using type = a;
};
template <typename func, typename a, typename bs>
struct foldl
    : detail::foldl_helper<func, a, typename detail::first_half<bs>::type,
                           typename detail::second_half<bs>::type> {};
template <typename func, typename a, typename bs>
struct foldl<func, a, list<bs> > : lambda<func>::template apply<a, bs>::type {};
}
#endif