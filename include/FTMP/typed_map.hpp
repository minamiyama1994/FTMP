...
namespace ftmp {
template <typename... T> class typed_map;
}
#include <utility>
#include "FTMP/at.hpp"
...
namespace ftmp {
template <typename k, typename a> class typed_map<list<k, a> > {
  a value_;
...
class typed_map<list<k, a>, T...> : public typed_map<T...> {
  static_assert(
      equal<typename size<typename unique<
                typename map<at<arg<0>, integral<int, 0> >,
                             list<list<k, a>, T...> >::type>::type>::type,
            typename size<list<list<k, a>, T...> >::type>::type::value,
      "key is a duplicate.");
  a value_;
...
public:
  ...
  auto operator=(typed_map &&) -> typed_map & = default;
  ~typed_map() = default;
  template <typename... T_> typed_map(const a &value, const T_ &... t);
  template <typename... T_> typed_map(a &&value, T_ &&... t);
  auto value() const -> const a &;
  auto value() -> a &;
  auto value(const a &value) -> void;
  auto value(a &&value) -> void;
...
template <typename k, typename a> auto typed_map<list<k, a> >::value() -> a &{
  return value_;
}
template <typename k, typename a>
...
typed_map<list<k, a>, T...>::typed_map(const a &value, const T_ &... t)
    : typed_map<T...>(t...), value_(value) {}
template <typename k, typename a, typename... T>
template <typename... T_>
typed_map<list<k, a>, T...>::typed_map(a &&value, T_ &&... t)
    : typed_map<T...>(std::move(t)...), value_(std::move(value)) {}
template <typename k, typename a, typename... T>
...
auto get(const typed_map<list<k_, a>, T...> &map)
    -> decltype(
          get_helper<equal<k, k_>::type::value, k, k_, a, T...>::func(map)) {
  return get_helper<equal<k, k_>::type::value, k, k_, a, T...>::func(map);
}
template <typename k, typename k_, typename a, typename... T>
auto get(typed_map<list<k_, a>, T...> &map)
    -> decltype(
          get_helper<equal<k, k_>::type::value, k, k_, a, T...>::func(map)) {
  return get_helper<equal<k, k_>::type::value, k, k_, a, T...>::func(map);
}
...
struct get_helper<false, k, k_, a, T...> {
  static auto func(const typed_map<list<k_, a>, T...> &map)
      -> decltype(get<k>(static_cast<const typed_map<T...> &>(map))) {
    return get<k>(static_cast<const typed_map<T...> &>(map));
  }
  static auto func(typed_map<list<k_, a>, T...> &map)
      -> decltype(get<k>(static_cast<typed_map<T...> &>(map))) {
