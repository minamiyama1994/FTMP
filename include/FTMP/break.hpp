...
namespace ftmp {
template <typename func, typename seq> struct break_;
}
...
struct break_helper
    : foldl<eval_if<eval<and_<ftmp::at<arg<0>, integral<int, 2> >,
                              not_<typename lambda<func>::template apply<
                                  arg<1> >::type> > >,
                    eval<list<eval<cons<id<arg<1> >,
                                        ftmp::at<arg<0>, integral<int, 0> > > >,
                              ftmp::at<arg<0>, integral<int, 1> >,
                              integral<bool, true> > >,
                    eval<list<ftmp::at<arg<0>, integral<int, 0> >,
                              eval<cons<id<arg<1> >,
                                        ftmp::at<arg<0>, integral<int, 1> > > >,
                              integral<bool, false> > > >,
            list<list<>, list<>, integral<bool, true> >, seq> {};
}
template <typename func, typename seq>
struct break_
    : map<reverse<arg<0> >,
          typename map<
              at<typename detail::break_helper<func, seq>::type, arg<0> >,
              list<integral<int, 0>, integral<int, 1> > >::type> {};
}
#endif