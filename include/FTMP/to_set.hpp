...
namespace ftmp {
template <typename T> struct to_set;
}
...
namespace ftmp {
namespace detail {
template <typename T> struct to_set_helper;
template <typename... T> struct to_set_helper<list<T...> > : set<T...> {};
}
template <typename T>
struct to_set : detail::to_set_helper<typename unique<T>::type> {};
}
#endif