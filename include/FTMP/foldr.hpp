...
namespace ftmp {
template <typename func, typename b, typename as> struct foldr;
}
...
struct foldr_helper;
template <typename func, typename b, typename as1, typename as2>
struct foldr_helper : foldr<func, typename foldr<func, b, as2>::type, as1> {};
}
template <typename func, typename b> struct foldr<func, b, list<> > {
  using type = b;
};
template <typename func, typename b, typename as>
struct foldr
    : detail::foldr_helper<func, b, typename detail::first_half<as>::type,
                           typename detail::second_half<as>::type> {};
template <typename func, typename b, typename as>
struct foldr<func, b, list<as> > : lambda<func>::template apply<as, b>::type {};
}
#endif